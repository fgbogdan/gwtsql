...
public class HourMinutePicker extends Composite {
	...
	private MouseOverHandler hourSelectedHandler = new MouseOverHandler() {
		...
		public void onMouseOver(MouseOverEvent event) {
			closePopupOnBlur = false;
			Widget source = (Widget) event.getSource();
			refreshWidget((workingSuffix == null || "".equals(workingSuffix)) ? "AM" : workingSuffix, source.getElement().getPropertyInt("hour"),
					workingMinute == -1 ? 0 : workingMinute);
		}
	};
	...
	private MouseOverHandler minuteSelectedHandler = new MouseOverHandler() {
		...
		public void onMouseOver(MouseOverEvent event) {
			closePopupOnBlur = false;
			Widget source = (Widget) event.getSource();
			refreshWidget((workingSuffix == null || "".equals(workingSuffix)) ? "AM" : workingSuffix, workingHour == -1 ? startHour : workingHour,
	...
	public void refreshWidget(String suffix, int hour, int minute) {
		...
		DOM.setStyleAttribute(pnlHoursAM.getElement(), "left", 2 - (pnlHoursAM.getOffsetWidth() / 2) - (pnlHoursAM.getOffsetWidth() % 2)
				+ (amSuffixLabel.getOffsetWidth() / 2) + (amSuffixLabel.getOffsetWidth() % 2) + "px");

		DOM.setStyleAttribute(pnlHoursPM.getElement(), "left", pmSuffixLabel.getElement().getOffsetLeft() + (pmSuffixLabel.getOffsetWidth() / 2)
		...
		if (workingHour != -1) {
			...
			DOM.setStyleAttribute(pnlMinutes.getElement(), "left",
					hourLabel.getAbsoluteLeft() - lblInput.getAbsoluteLeft() - (pnlMinutes.getOffsetWidth() / 2) + (hourLabel.getOffsetWidth() / 2) + "px");
		} else {
			DOM.setStyleAttribute(pnlMinutes.getElement(), "left", 2 - (pnlMinutes.getOffsetWidth() / 2) - (pnlMinutes.getOffsetWidth() % 2)
					+ (amSuffixLabel.getOffsetWidth() / 2) + (amSuffixLabel.getOffsetWidth() % 2) + "px");
		}
